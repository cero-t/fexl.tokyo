...
{{% panel theme="default" header="(86) しゃがみ中K→肘激崩(中)→掌撃破→連昇激→(しゃがみ弱P)→ダッシュ中K→弱P→ダッシュ弱P→しゃがみ中P→弱P→ダッシュ弱P→中P→強P→刈蹴→連昇激→J強K→中P→強K" %}}刻みコンボから連昇激を入れてダメージを伸ばせる。ゲージも1/3程度は回復できる。{{% /panel %}}
...
{{% panel theme="default" header="(105) ダッシュしゃがみ弱K→しゃがみ中K→しゃがみ弱K→しゃがみ中K→肘激崩(中)→掌撃破→連昇激→J強K→ダッシュ中P→強K→連昇激→J強K→ダッシュ弱K→中P→強P→連昇激→振脚撃(強)-気錬射" %}}弱中弱中から始めた場合のコンボ。インフィニティでない場合は、最初の連昇激の後に入れている{{% /panel %}}
